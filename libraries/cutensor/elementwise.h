...
namespace _register_ew_same_shape {
  ...
  struct op {
    ...
    void operator()(
      ...
      handle_error("cutensorPermutation", cutensorPermutation(
        // handle
        &params.cutensor_handle, 
        // input 0
...
namespace _register_ewb_ij_i {
  ...
  struct op {
    ...
    void operator()(
      ...
      cutensorTensorDescriptor_t desc0;
      handle_error("cutensorInitTensorDescriptor", cutensorInitTensorDescriptor( 
          &params.cutensor_handle, 
          &desc0, 
          2,
      ...
      // Note that input 0 and input 1 are swapped in the call to cutensor.
      // This is because cutensor has a restriction that desc0 be the same
      // for it's snd input and output.
      // so instead of op( alpha*A(ij), beta*B(i)   ) -> (ij)
      // we do         op( beta*B(i),   alpha*A(ij) ) -> (ij)
      handle_error("cutensorElementwiseBinaryIJI", cutensorElementwiseBinary(
        // handle
        &params.cutensor_handle, 
        // input 1
        (void*)&beta,  data1, &desc1, mode_names,
        // input 0
        (void*)&alpha, data0, &desc0, mode_names,
        // output
                       data2, &desc0, mode_names,
        // no output op; which scalar type
        scalar_op, cutensor_scalar_type,
        // the stream
        params.stream));
...
// NOTE: only call this if scalar_op is commutative!!!!
void register_ewb_ij_i(
  udf_manager_ptr udf_manager,
  std::string name,
  cutensorOperator_t scalar_op, 
  float alpha, 
  ...
  // TODO: verify that scalar_op is commutative
  ...
  udf_manager->register_udf_impl(
    std::make_unique<_register_ewb_ij_i::f>(name, alpha, beta, scalar_op));
}
